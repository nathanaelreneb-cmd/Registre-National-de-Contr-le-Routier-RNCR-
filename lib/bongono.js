...
export function demarrerGuidage(setVitesse, setStatus) {
  ...
  navigator.geolocation.watchPosition(async (pos) => {
    ...
    let zonesCache = []
    try {
      zonesCache = JSON.parse(localStorage.getItem('bongono_zones') || '[]')
    } catch(e) {}
    ...
    try {
      ...
      if (zones && zones.length > 0) {
        zonesCache = zones
        localStorage.setItem('bongono_zones', JSON.stringify(zones))
      }
    } catch(e) {}
...
export async function ajouterZone(lat, lon, message, ville, type) {
  // --- SÉCURITÉ FAYA : SEULEMENT AGENTS / POLICE ---
  const { data: { user } } = await supabase.auth.getUser()
  if (!user) return alert("⛔ Connecte-toi d'abord")

  const { data: profil } = await supabase.from('profiles').select('role').eq('id', user.id).single()
  
  if (!['admin','responsable','agent'].includes(profil?.role)) {
    return alert("⛔ Accès réservé aux agents et policiers")
  }
