...
// Fonction pour que les agents ajoutent une zone dangereuse
export async function ajouterZone(lat, lon, message, ville) {
  const { error } = await supabase.from('bongono_zones').insert([{ lat, lon, message, ville }])
  if (error) alert(error.message)
  else alert("Zone ajoutée avec succès pour " + ville)
      }
