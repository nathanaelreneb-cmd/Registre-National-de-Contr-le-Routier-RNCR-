...
export default function AdminBongono() {
  ...
  const [pos, setPos] = useState(null) // pour afficher les coordonnées
  ...
  async function handleMapClick(lat, lng){
    setPos({lat, lng}) // <-- On garde les coordonnées à l'écran
  }
  ...
  async function confirmerPanneau(){
    ...
    alert('✅ Panneau posé à '+pos.lat.toFixed(5)+', '+pos.lng.toFixed(5))
  }
  ...
  return (
    <div style={{padding:15}}>
      ...
      <div style={{display:'flex', gap:8, marginBottom:10, flexWrap:'wrap'}}>
        ...
        <input value={message} onChange={e=>setMessage(e.target.value)} placeholder="Ex: Travaux de route" style={{padding:10, flex:1}} />
      </div>

      {pos && (
        <div style={{background:'#e8f5e9', padding:10, borderRadius:8, marginBottom:10, border:'1px solid green'}}>
          📍 Sélectionné: <b>{pos.lat.toFixed(6)}, {pos.lng.toFixed(6)}</b><br/>
          Le panneau va s'afficher là-bas<br/>
          <button onClick={confirmerPanneau} style={{marginTop:8, padding:'10px 15px', background:'green', color:'white', border:'none', borderRadius:8, width:'100%', fontWeight:'bold'}}>✅ CONFIRMER LE PANNEAU ICI</button>
        </div>
      )}

      <MapComponent onMapClick={handleMapClick} zones={zones} selectedPos={pos} />
      <p style={{marginTop:8}}>Clique sur la carte pour repérer les coordonnées, puis confirme.</p>
    </div>
  )
          }
