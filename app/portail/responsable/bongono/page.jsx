...
export default function AdminBongono() {
  ...
  async function confirmer(){
    ...
    try {
      const { createClient } = await import('@supabase/supabase-js')
      const supa = createClient(process.env.NEXT_PUBLIC_SUPABASE_URL, process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY)
      // ON A ENLEVÉ "active" ICI - C'EST ÇA QUI PLANTAIT
      const { error } = await supa.from('bongono_zones').insert({ lat: pos.lat, lng: pos.lng, type, message })
  ...
  return (
    <div style={{padding:12}}>
      ...
      {zones.map(z=>(
        <div key={z.id} style={{border:'1px solid #ddd', padding:10, borderRadius:8, marginBottom:6, display:'flex', justifyContent:'space-between'}}>
          <div><b>{z.type}</b> - {z.message}<br/><small>{Number(z.lat).toFixed(5)}, {Number(z.lng).toFixed(5)}</small></div>
