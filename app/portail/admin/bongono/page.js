...
export default function PageBongonoAdmin() {
  ...
  async function handleSubmit(e) {
    e.preventDefault()
    if (!lat || !lon || !message) return alert('Remplis tout')
    await ajouterZone(parseFloat(lat), parseFloat(lon), message, ville, type)
    setLat(''); setLon(''); setMessage('')
    chargerZones()
  }
  ...
  return (
    <div style={{ padding: 20, maxWidth: 600, margin: '0 auto' }}>
      ...
      <p>Ajouter une zone dangereuse</p>
      ...
      <form onSubmit={handleSubmit} style={{ background: '#f5f5f5', padding: 15, borderRadius: 10, marginTop: 15 }}>
        ...
        <select value={type} onChange={e=>setType(e.target.value)} style={{ width: '100%', padding: 8, marginTop: 8 }}>
          ...
          <option value="dos_dane">dos_dane</option>
        </select>
        <button type="submit" style={{ marginTop: 12, background: 'black', color: 'white', padding: '10px 15px', borderRadius: 8, width: '100%' }}>
          Ajouter la zone
        </button>
        <p style={{ fontSize: 12, marginTop: 8 }}>Astuce: Va sur Google Maps, clic droit sur un point, copie les coordonnées</p>
      </form>

      <h3 style={{ marginTop: 20 }}>Zones sauvegardées ({zones.length})</h3>
      <div>
        {zones.map(z => (
          <div key={z.id} style={{ border: '1px solid #ddd', padding: 8, borderRadius: 8, marginTop: 8 }}>
            <b>{z.type}</b> - {z.ville} - {z.message}
            <br/>
            <small>{z.lat}, {z.lon}</small>
          </div>
        ))}
      </div>
    </div>
